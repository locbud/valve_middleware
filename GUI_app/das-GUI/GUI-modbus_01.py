...
# Timer is from Here!
time1 = ''
clock = Label(das, font=('times', 15, 'bold'), bg='#FDFEFE', padx = 200, pady = 20)
clock.grid(row=1,columnspan=3)


slaveID = Label(das, text=' Slave ID: 3 ', font=('times', 15, 'bold'), fg='red', bg='#FDFEFE', padx = 10, pady = 10)
slaveID.grid(row=1,column=0)

ValveState = Label(das,text='Vavle', font = Font1, fg='#21618C', bg = '#FDFEFE', padx = 10, pady = 10)
...
def ledON():
    ...
    onoff = Label(das,text=' ON ', font = Font3, bg = '#FDFEFE', fg='green', padx = 10)
    onoff.grid(row=1,column=2, sticky = E)

def ledOFF():
    #GPIO.output(LED, GPIO.LOW) # led off
    onoff = Label(das,text='OFF', font = Font3, bg = '#FDFEFE', fg='red', padx = 10)
    onoff.grid(row=1,column=2, sticky = E)
###########################################################
...
Text11 = Label(das,text='Room Temperature', font = Font1, fg='#21618C', bg = '#FDFEFE', padx = 10, pady = 10)
Text11.grid(row=6,column=0)
roomTemp = Label(das, font = Font1, fg='#21618C', bg = '#FDFEFE', padx = 10, pady = 10)
roomTemp.grid(row=6,column=1)
roomTempUnit = Label(das, text= '\N{DEGREE SIGN}C', font = Font1, fg='#21618C', bg = '#FDFEFE')
roomTempUnit.grid(row=6,column=1,  sticky = E)
...
thermoTempH = Label(das, font = Font1, fg='#21618C', bg = '#FDFEFE')
thermoTempH.grid(row=2,column=1, padx = 1, pady = 1)
degreeSign = Label(das, text= '\N{DEGREE SIGN}C', font = Font1, fg='#21618C', bg = '#FDFEFE')
degreeSign.grid(row=2,column=1,  sticky = E)

thermoTempL = Label(das, font = Font1, fg='#21618C', bg = '#FDFEFE')
thermoTempL.grid(row=3,column=1, padx = 1, pady = 1)
percent = Label(das, text= '\N{DEGREE SIGN}C', font = Font1, fg='#21618C', bg = '#FDFEFE')
...
def modtk():
    ...
    try:
       ...
       # logger.info(master.execute(3, cst.READ_INPUT_REGISTERS, 2, 8 , data_format='>ffff'))
        ...
        thermoTempH.config(text=reg1)
        thermoTempL.config(text=reg2)
        Text8.config(text=reg3)
        Text10.config(text=reg4)
        #####
    except modbus_tk.modbus.ModbusError as exc:
        logger.error("%s- Code=%d", exc, exc.get_exception_code())

    thermoTempH.after(3000, modtk)
modtk ()
...
Button1 = Button(das, text=' ON ',command = ledON, font = Font1, bg='bisque2', height = 1, width = 10)
...
Button2 = Button(das, text=' OFF ', command = ledOFF,font = Font1, bg='bisque2', height = 1, width = 10)
